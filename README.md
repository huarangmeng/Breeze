...
- `BreezeTheme` 主题 token 已接通
- `WindowInfo` 响应式分级已接通
- `:data` 的 Ktor / Room3 / Settings / Coil3 已接通
- `ChatRoute` + `ChatViewModel` 已接入 `Koin`，可通过仓库内 `MockEngine` 完成“发送消息 -> 回显 -> 持久化”的最小闭环
- `HistoryRoute` + `HistoryViewModel` 已接通真实会话与消息流，可浏览历史列表和详情摘要
- `M5` 已补 `LlmProviderId`、`ModelProfile`、`LlmProviderRegistry` 与 `LocalProvider` 主干，发送链路开始走 provider 抽象
- `ApiConfigRoute` + `ApiConfigViewModel` 已接通 Provider 选择、Endpoint、API Key 持久化，当前 Hosted Provider 先走兼容桥接
- `ModelSettingsRoute` + `ModelSettingsViewModel` 已接通模型选择骨架，当前可持久化模型 ID，参数区与预设区保留给 `M5-8`
- `BreezeNavHost` 已统一根布局：`Compact/Medium` 维持单栏顶部导航，`Expanded` 切为左侧 feature 列表 + 右侧详情区
- `M4-8` 已完成评估：当前先不拆 `:feature:*` 模块，等待更强的依赖/协作信号再按 ADR 0003 执行
- `docs/design/navigation.md` 已补齐当前路由表、根布局、返回栈与深链边界

下一阶段建议优先补齐以下能力：

- 真实 OpenAI / Anthropic Provider 接入
- 模型参数持久化与预设管理
- `M5` 后继续观察是否触发 feature 模块拆分条件
