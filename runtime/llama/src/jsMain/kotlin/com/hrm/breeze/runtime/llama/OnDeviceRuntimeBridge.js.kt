    ...
    object : OnDeviceRuntimeBridge {
        override val capability =
            unsupportedLlamaCapability(
                reason = "llama.cpp runtime is not implemented on JS yet",
                family = com.hrm.breeze.runtime.api.OnDeviceRuntimeFamily.WebAssembly,
                targetPlatforms = setOf(com.hrm.breeze.runtime.api.OnDeviceRuntimeTargetPlatform.Web),
                supportsModelPersistence = false,
            )
        ...
        override fun streamCompletion(request: OnDeviceRuntimeCompletionRequest): Flow<String> = flow {
            error("llama.cpp runtime is not implemented on JS yet")
        }
    }
